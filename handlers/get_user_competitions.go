...
type TeamResponse struct {
	...
	RemainingSlot int                `json:"remainingSlot"`
...
func GetUserCompetitionsHandler(db *sqlx.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// CHANGED: Extract the user ID (UUID) from the "sub" claim instead of just the email
		...
		// CHANGED: Query using rcm.user_id = $1
		err := db.Select(&teams, `
        ...
        `, userID)
		if err != nil {
			fmt.Println("DB ERROR (fetch teams):", err) // Helpful for debugging
		...
		for _, t := range teams {
			...
			members := make([]models.User, 0) // FIX: This ensures it returns [] instead of null
		...
		// Return empty array instead of null if user has no teams
		if result == nil {
			result = []TeamResponse{}
		}
